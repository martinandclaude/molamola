...
def make_karyotype_report(
    ...
    *,
    ...
    html = (
        '<!DOCTYPE html>\n<html lang="en"><head>\n<meta charset="UTF-8">\n'
        f'<title>molamola karyotype &mdash; {sample_e}</title>\n'
        f"<style>{_HTML_REPORT_CSS}</style>\n"
        '</head>\n<body>\n'
        f'<header>\n{_header_svg()}</header>\n'
        ...
        f'</details>\n'
        f'<footer>generated by <code>molamola</code> '
        f'&middot; karyotype mode &middot; '
        f'mosdepth via <a href="https://github.com/brentp/mosdepth">'
        f'mosdepth</a></footer>\n'
        '</body></html>\n'
    )
    out_path.write_text(html)
...
def _add_karyotype_args(p) -> None:
    ...
    p.add_argument(
        ...
        help="override the bundled exclusion mask. Default: "
             "data/exclusion.<reference>.bed.gz. "
             "Bins overlapping any interval here are dropped from "
             "the CN scatter, smooth, and normalisation.",
    )
    p.add_argument(
        "--no-mask", action="store_true",
        help="skip masking entirely (sensitivity check; not recommended "
    ...
    p.add_argument(
        ...
        help="override the bundled 10 kb GC table. Default: "
             ...
             "GC bucket median ratio depth correction.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--centromere-pad-kb", type=float, default=1000.0,
        help="extend the exclusion mask by this many kb on each side of "
             "every acen band. Default 1000 (1 Mb).",
    )
    p.add_argument(
        "--scatter-bin-kb", type=float, default=50.0,
        help="target bin width (kb) for the coverage-scatter cloud. "
             "Default 50.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="hard cap on coverage-scatter points (per figure). "
             "Default 200000.",
    )
    p.add_argument(
        "--max-baf-points", type=int, default=80_000,
        help="hard cap on BAF-scatter points (genome-wide panel). "
             "Default 80000.",
    )
    p.add_argument(
        "--min-baf-dp", type=int, default=10,
        help="minimum FORMAT/DP for a het site to enter the BAF panel. "
             "Default 10.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="genomic sex used to draw the expected-CN dashes for "
             "chrX / chrY. 'auto' (default) calls male iff chrY "
             "median CN > 0.3 after normalisation.",
    )
...
def karyotype_main(args: argparse.Namespace) -> int:
    ...
    1. Resolve refs (cytoband + mask + GC; bundled-by-default).
    2. Load mosdepth + cytoband. Refuse non-uniform bins.
    3. Compute ``mask_pass = NOT (exclusion-mask OR centromere-pad)``.
    4. GC-correct depth (per-1 % GC bucket median ratio).
    5. Anchor CN at 2.0 on the autosomal non-masked median.
    6. Infer genomic sex (or honour ``--sex``).
    ...
    if hint is not None and hint != args.reference:
        msg = (f"mosdepth filename '{args.mosdepth.name}' suggests "
               f"--reference {hint} but --reference {args.reference} "
               f"was given")
        if args.force:
            print(f"  WARNING: {msg}. Continuing because of --force.",
                  file=sys.stderr)
    ...
    sample = args.sample or (
        args.mosdepth.name
        .replace(".regions.bed.gz", "")
        .replace(".bed.gz", "")
        .replace(".bed", "")
    )

    cb_path = find_cytoband_file(args.reference)
    mask_path: Path | None
    if args.no_mask:
        mask_path = None
        ...
        mask_path = find_mask_file(args.reference)
    gc_path: Path | None
    if args.no_gc:
        gc_path = None
    elif args.gc is not None:
        gc_path = args.gc
    else:
        gc_path = find_gc_file(args.reference)

    print(f"reading mosdepth: {args.mosdepth}")
    try:
        cov, bin_size = read_mosdepth(args.mosdepth)
        ...
        return 1
    print(
        f"[info] mosdepth: {len(cov):,} bins at "
        f"{_kary_format_bin_size(bin_size)}",
    )

    cb = read_cytoband_df(cb_path)
    ...
    if mask_path is not None:
        ...
        mask_label = "off"
    centro_excluded = annotate_centromere_pad(
        cov, cb, lengths, args.centromere_pad_kb,
    )
    mask_pass = ~(excluded | centro_excluded)
    n_masked = int((~mask_pass).sum())
    pct_masked = 100.0 * n_masked / max(len(cov), 1)
    print(
        f"[info] mask: {n_masked:,} / {len(cov):,} bins "
        f"({pct_masked:.1f} %) excluded",
    )

    depth = cov["depth"].to_numpy().astype(np.float64)
    if gc_path is not None:
        gc_pct = annotate_gc(cov, gc_path, gc_bin_size=10_000)
        factors = fit_gc_correction(depth, gc_pct, mask_pass, cov["chrom"])
        depth_corrected = apply_gc_correction(depth, gc_pct, factors)
        gc_label = gc_path.name
    else:
        depth_corrected = depth
        gc_label = "off"

    autosomal = cov["chrom"].isin(KARY_AUTOSOMES).to_numpy()
    anchor_use = autosomal & mask_pass & (depth_corrected > 0)
    if not anchor_use.any():
        print(
            "ERROR: no autosomal non-masked bins to anchor CN. Check "
            "that the mosdepth file covers autosomes and that the "
            "exclusion mask is not over-aggressive.",
            file=sys.stderr,
        )
        return 1
    autosomal_median = float(np.median(depth_corrected[anchor_use]))
    cn = depth_corrected * 2.0 / autosomal_median
    cov2 = cov.copy()
    cov2["depth_corrected"] = depth_corrected
    cov2["cn"] = cn
    ...
    if args.sex == "auto":
        ...
        sex = args.sex
        print(f"[info] genomic sex: {sex}  (--sex {args.sex})")

    cov2 = rolling_median_per_chrom(
        cov2, bin_size=bin_size,
        window_mb=args.smooth_window_mb, mask_pass=mask_pass,
    )
    ...
    make_karyotype_report(
        ...
        smooth_window_mb=args.smooth_window_mb,
