...
- **VCF inputs are header-driven.** `##INFO=<ID=SVTYPE,...>` selects the SV / cytogenetics report. Any other VCF shape is refused rather than plotted on a guess.
...
Either `--vcf` or `--mosdepth` is required. Mode-specific flags are silently ignored when they don't apply to the active mode (e.g. passing `--bin-size` against a mosdepth run — karyotype mode just doesn't read it).
...
|---|---|---|
| `--vcf PATH` | — | input VCF (gzipped OK). Required unless `--mosdepth` is given. Selects SV mode by header; consumed only as the BAF source when `--mosdepth` is also given. |
